...
def main():
    ...
    rows = load_flow_rows(con, a.flow)          # country_obs + inf-only sections
    # label passes (fused splits, phantom relabel); repairs are keyed on the
    # RAW parse, so they are looked up on `rows`, not on `fixed`
    fixed = relabel(rows, a.flow)
